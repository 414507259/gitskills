...
def get_stock_realtime_data(stock_codes):
    ...
    if all_data:
        ...
        print(f"成功获取 {len(result_df)} 只股票的实时数据")
        return result_df
    return None
...
def analyze_and_filter_stocks():
    ...
    # 转换流通市值为亿元
    if '流通市值' in realtime_df.columns:
        realtime_df['流通市值亿'] = realtime_df['流通市值'] / 1e8
...
def generate_report(filtered_df, filename):
    ...
    report_date_file = datetime.now().strftime("%Y%m%d")
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        if filtered_df is None or len(filtered_df) == 0:
            ...
            # 显示列
            display_cols = ['代码', '名称', '最新价', '涨跌幅', '市盈率-动态', 
                          '市净率', '换手率', '流通市值亿', '综合评分']
