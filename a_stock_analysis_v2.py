...
def analyze_and_filter_stocks():
    ...
    # 获取所有A股实时数据
    all_stock_df = get_all_a_stock_data()
    if all_stock_df is None or len(all_stock_df) == 0:
        print("无法获取实时行情数据")
        return None
    
    # 筛选沪深300成分股
    hs300_codes = set(hs300_df['品种代码'].astype(str).tolist())
    filtered_df = all_stock_df[all_stock_df['代码'].astype(str).isin(hs300_codes)].copy()
    print(f"\n筛选出 {len(filtered_df)} 只沪深300成分股的实时数据")
