...
def get_stock_realtime_data(stock_codes):
    """获取个股实时行情数据"""
    print(f"正在获取 {len(stock_codes)} 只股票实时数据...")
    try:
        df = ak.stock_zh_a_spot_em()
        # 确保代码格式一致
        stock_codes_normalized = [str(code).zfill(6) for code in stock_codes]
        hs300_data = df[df['代码'].isin(stock_codes_normalized)].copy()
        print(f"成功获取 {len(hs300_data)} 只股票实时数据")
        return hs300_data
    except Exception as e:
        print(f"获取实时数据失败: {e}")
        return pd.DataFrame()
...
def main():
    ...
    # 获取实时数据
    realtime_df = get_stock_realtime_data(stock_codes)
    if realtime_df.empty:
        print("无法获取实时行情数据，程序退出")
        return
    ...
    if not filtered_df.empty:
        print(f"\n摘要: 找到 {len(filtered_df)} 只符合买入条件的股票")
